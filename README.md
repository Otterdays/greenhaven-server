...
Per-tag base URL (example tag `v0.1.0`):

```
https://github.com/Otterdays/greenhaven-server/releases/download/v0.1.0/greenhaven-server-x86_64-unknown-linux-musl
https://github.com/Otterdays/greenhaven-server/releases/download/v0.1.0/greenhaven-server-x86_64-unknown-linux-musl.sha256
https://github.com/Otterdays/greenhaven-server/releases/download/v0.1.0/greenhaven.json
```

## Verify and run

```sh
BASE=https://github.com/Otterdays/greenhaven-server/releases/download/v0.1.0
...
RPG_BIND=0.0.0.0:6000 RPG_MAP=assets/maps/greenhaven.json RPG_DB=data/world.sqlite ./greenhaven-server
```

It prints `world listening on ws://...` once it is up. Exactly one TCP port is used: the WebSocket `/ws` and `GET /health` share it. `/debug/world` stays off on a non-loopback bind unless `RPG_DEBUG=1`. Stop is `^C`; the server also flushes on SIGTERM.
